...
def main():
    ...
    with pm.Model() as logistic_model:
        ...
        pi = pm.Deterministic('pi', pm.math.sigmoid(beta0 + beta1*x1 + beta2*x2)) #functia sigmoid, probabilitatea de admitere

        admission_obs = pm.Bernoulli('admission_obs', p=pi, observed=admission)
        trace = pm.sample(2000, tune=1000)

    pm.summary(trace) # tabelul cu rezultatele (hdi-ul, mean, sd, etc.)
    ...
    p_grid = pm.math.sigmoid(trace.posterior['beta0'].values + trace.posterior['beta1'].value * x_grid[0] + trace.posterior['beta2'].value * x_grid[1])
